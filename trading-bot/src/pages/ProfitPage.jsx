...
function SummaryCard({ value, label, colorClass }) {
  return (
    <div className={`${colorClass} rounded-xl p-6 hover:brightness-110 transition-all cursor-default`}>
      ...
      <div className="text-gray-400 text-[13px] font-bold tracking-wide uppercase">{label}</div>
    </div>
  );
}
...
export default function ProfitPage() {
  ...
  return (
    <div className="flex flex-col w-full h-full pb-8 gap-6">
      ...
      <div className="grid grid-cols-3 gap-5">
        ...
        <div className="bg-[#a855f7]/10 border border-[#a855f7]/20 rounded-xl p-6 hover:brightness-110 transition-all cursor-default">
          <div className="flex items-end gap-3 mb-2">
            ...
            <div className="text-emerald-400 text-[13px] font-bold pb-[3px] bg-emerald-500/10 px-2 rounded tracking-wide">
      ...
      {/* Weekly trend chart */}
      <section className="bg-[#1a1d23] border border-[#2a2d33] rounded-xl shadow-lg p-6 flex-shrink-0">
      ...
      {/* Closed deals table */}
      <section className="bg-[#1a1d23] border border-[#2a2d33] rounded-xl flex flex-col overflow-hidden shadow-lg flex-1">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#2a2d33]">
          ...
          <button
            ...
            className="px-4 py-2 text-[13px] font-bold text-gray-300 border border-[#3a3d43] rounded-lg hover:bg-white/[0.05] hover:text-white transition-all active:scale-[0.98]"
          >
            Export CSV
          </button>
        </div>
            ...
            {sortedDeals.length === 0 ? (
              ...
              <tbody className="divide-y divide-[#2a2d33]/50">
                {sortedDeals.map((deal, idx) => (
                  <tr key={deal.inquiry_id} className={`h-[56px] hover:bg-white/[0.04] transition-colors ${idx % 2 !== 0 ? 'bg-[#242830]/20' : ''}`}>
                    ...
                    <td className="px-6 text-right font-mono text-gray-400">{formatINR(deal.seller_cost)}</td>
                    <td className="px-6 text-right font-mono text-white font-semibold">{formatINR(deal.my_price)}</td>
                    <td className="px-6 text-center font-mono text-gray-400 font-bold">{(deal.margin_percent ?? 0).toFixed(1)}%</td>
                    <td className="px-6 text-right font-mono text-emerald-400 font-bold">{formatINR(deal.profit)}</td>
                    <td className="px-6 pl-8 text-gray-500 text-[13px]">{formatDateString(deal.date_closed)}</td>
                  </tr>
                ))}
                {/* Totals row */}
                <tr className="bg-[#0f1117]/80 hover:bg-[#0f1117] transition-colors">
                  ...
                  <td className="px-6 py-5 text-right font-mono font-bold text-gray-300">{formatINR(totalCost)}</td>
                  <td className="px-6 py-5 text-right font-mono font-bold text-white">{formatINR(totalRevenue)}</td>
                  <td className="px-6 py-5 text-center font-mono font-bold text-gray-400">{marginPercent.toFixed(1)}%</td>
                  <td className="px-6 py-5 text-right font-mono font-bold text-emerald-400 text-[15px]">{formatINR(totalProfit)}</td>
