...
export default function InventoryPage() {
  ...
  const [inventory, setInventory] = useState([
    ...
    { id: 'INV-1003', name: 'Circuit Boards', category: 'Electronics', quantity: 0, price: '$150.00', location: 'Warehouse A', status: 'Out of Stock' },
    { id: 'INV-1004', name: 'Hydraulic Fluid', category: 'Consumables', quantity: 200, price: '$85.00', location: 'Warehouse C', status: 'In Stock' },
  ]);
  ...
  const handleSave = (formData) => {
    if (editItem) {
      setInventory(inventory.map(item => item.id === editItem.id ? { ...item, ...formData } : item));
    } else {
      setInventory([...inventory, { id: `INV-${1000 + inventory.length + 1}`, ...formData }]);
    }
    setIsFormOpen(false);
    setEditItem(null);
  };
  ...
  return (
    <div className="flex flex-col w-full h-full pb-4">
      <div className="w-full flex-1 flex flex-col mt-4">
        ...
        <RightDrawer isOpen={isFormOpen} title={editItem ? 'Edit Inventory Item' : 'Add New Item'} onClose={() => { setIsFormOpen(false); setEditItem(null); }}>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-[#2a2d33] text-gray-700 dark:text-gray-300">
                {filteredInventory.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">
                    ...
                    <td className="px-5 py-3"><span className="px-2 py-1 bg-gray-100 dark:bg-[#2a2d36] rounded text-[11px] font-bold text-gray-600 dark:text-gray-400">{item.category}</span></td>
                    ...
                    <td className="px-5 py-3 text-right space-x-3">
                      ...
                      <button onClick={() => { setEditItem(item); setIsFormOpen(true); }} className="text-blue-500 hover:text-blue-600 transition-colors" title="Edit"><EditIcon /></button>
                      <button onClick={() => handleDelete(item.id)} className="text-red-500 hover:text-red-600 transition-colors" title="Delete"><TrashIcon /></button>
                    </td>
                  </tr>
                ))}
