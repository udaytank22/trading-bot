...
export function RightDrawer({ isOpen, title, onClose, children, isFullScreen }) {
  ...
  return (
    <div className={`fixed inset-0 z-[100] flex items-center justify-center ${isFullScreen ? "" : "p-4"}`}>
      ...
      <div className={`relative w-full bg-white dark:bg-[#1a1d23] shadow-2xl flex flex-col overflow-hidden border border-gray-200 dark:border-[#2a2d33] animate-fade-in ${isFullScreen
        ? "h-full max-w-full"
...
export function ViewDetails({ item, onClose }) {
  ...
  const [previewFile, setPreviewFile] = useState(null); // { file, name }
  ...
  return (
    <div className="flex flex-col h-full">
                    ...
                    value.map((v, i) => (
                      <span key={i} className="px-2 py-0.5 bg-purple-50 dark:bg-purple-900/20 text-purple-600 dark:text-purple-400 rounded text-xs font-semibold border border-purple-500/10 flex items-center gap-1">
                        {typeof v === 'object' && v !== null ? (
                          v.file ? (
                            <button type="button" onClick={() => setPreviewFile({ file: v.file, name: v.name || 'Document' })} className="hover:underline flex items-center gap-1 text-purple-700 dark:text-purple-300">
      ...
      <CenterModal isOpen={!!previewFile} title={previewFile?.name || 'Document Preview'} onClose={() => setPreviewFile(null)}>
        {previewFile && (
          <div className="flex justify-center items-center w-full bg-gray-50 dark:bg-[#0f1117] rounded-lg p-2 min-h-[50vh]">
            {previewFile.file.startsWith('data:image/') ? (
              <img src={previewFile.file} alt="Preview" className="max-w-full max-h-[75vh] object-contain rounded" />
            ) : previewFile.file.startsWith('data:application/pdf') ? (
              <iframe src={previewFile.file} className="w-full h-[75vh] rounded border-0" title="PDF Preview" />
            ) : (
              <iframe src={previewFile.file} className="w-full h-[75vh] rounded border-0 bg-white" title="Document Preview" />
            )}
          </div>
        )}
      </CenterModal>
...
export function HeaderButton({ onClick, icon, children, className = '' }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`inline-flex items-center justify-center gap-2 px-5 py-2.5 text-sm font-bold text-[#1e293b] dark:text-white bg-[#faf8f5] dark:bg-[#1a1d23] hover:bg-[#f2ede2] dark:hover:bg-[#242830] border border-[#e6e0d2] dark:border-[#2a2d33] rounded-2xl shadow-sm hover:shadow transition-all whitespace-nowrap flex-shrink-0 cursor-pointer ${className}`}
    >
      {icon}
      {children}
    </button>
