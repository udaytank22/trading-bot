        ...
        renderRow={(client, i) => (
          <tr key={client.id} className={`${rowStripeClass(i)} ${ROW_HOVER_CLS}`}>
            ...
            <td className="px-5 py-3 text-right space-x-3">
              ...
              {canUpdate && <button onClick={() => { setEditItem(client); setIsFormOpen(true); }} className="text-blue-500 hover:text-blue-600 transition-colors" title="Edit"><EditIcon /></button>}
              {canDelete && <button onClick={() => handleDelete(client.id)} className="text-red-500 hover:text-red-600 transition-colors" title="Delete"><TrashIcon /></button>}
            </td>
          </tr>
        )}
        paginationProps={{
