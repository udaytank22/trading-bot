...
export default function InquiryDetailsPage() {
  ...
  const handleCloseRFQ = async () => {
    ...
    try {
      ...
      if (res.success) {
        refreshAll();
        setActiveTab("overview");
        Swal.fire({ icon: 'success', title: 'RFQ Closed', text: 'Inquiry is now in TL Review.', background: '#1a1d23', color: '#fff', showConfirmButton: false, timer: 1500 });
      }
    } catch (err) {
      console.error(err);
  ...
  const handleSelectSupplierQuote = async (quoteId) => {
    ...
    try {
      ...
      if (res.success) {
        setDeal(res.data);
        Swal.fire({ icon: 'success', title: 'Quote Selected', text: 'Supplier quote selected and client quote updated.', background: '#1a1d23', color: '#fff', showConfirmButton: false, timer: 1500 });
      }
    } catch {
  ...
  const handleSelectSupplierQuoteItem = async (quoteItemId, productName) => {
    ...
    try {
      ...
      if (res.success) {
        setDeal(res.data);
        Swal.fire({ icon: 'success', title: 'Selection Saved', text: `${productName} will be sourced from this supplier.`, background: '#1a1d23', color: '#fff', showConfirmButton: false, timer: 1600 });
      }
    } catch {
  ...
  const handleConfirmSourcing = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setExpandedQuotes({});
        setActiveTab('action');
        Swal.fire({ icon: 'success', title: 'Sourcing Confirmed!', text: 'Client quotation has been updated.', background: '#1a1d23', color: '#fff', showConfirmButton: false, timer: 2000 });
      }
    } catch {
